...
//
// FIX (black cards):
//  - /api/events ВСЕГДА подтягивает items по всем стадиям
//  - "цветные" стадии = STAGE_FILTER (или ?stages=...)
//  - остальные стадии НЕ отфильтровываются: показываются как ЧЁРНЫЕ
//  - ?allStages=1 отключает чернение (все стадии будут цветными)
...
// по умолчанию считаем "цветными" эти стадии
const STAGE_FILTER = [STAGE_CREATE, STAGE_READY, STAGE_SUCCESS];

// "чёрные" карточки (прочие стадии)
const BLACK_COLOR = '#111827';   // графит/чёрный
const BLACK_SORTKEY = 8;         // после всех цветных
...
// ==== сортировка внутри дня: green -> blue -> purple -> red -> gray -> black ====
function colorToSortKey(color) {
  ...
  if (c === BLACK_COLOR) return BLACK_SORTKEY;         // чёрные
  return 9;
}
...
// opts:
//  - paintOtherStagesBlack: true/false
//  - coloredStages: Set(stageId) — какие стадии считать "цветными"
function mapItemsToEvents(items, opts = {}) {
  const events = [];
  const skipped = { noDate: 0 };

  const paintOtherStagesBlack = !!opts.paintOtherStagesBlack;
  const coloredStages = opts.coloredStages instanceof Set ? opts.coloredStages : new Set(STAGE_FILTER);
  ...
  for (const item of items) {
    ...
    // ✅ ключевая логика "чёрных"
    const inColored = coloredStages.has(stageId);

    const color = (paintOtherStagesBlack && !inColored)
      ? BLACK_COLOR
      : pickColorByStoneAndPickup({
          stageId,
          stoneText,
    ...
    const sortKey = (paintOtherStagesBlack && !inColored)
      ? BLACK_SORTKEY
      : colorToSortKey(color);
    ...
    events.push({
      ...
      extendedProps: {
        color,
        sortKey,
        isBlack: (paintOtherStagesBlack && !inColored),
...
app.get('/api/events', async (req, res) => {
  ...
  try {
    ...
    // ?stages=... — какие стадии считать "цветными"
    const stagesRaw = String(req.query.stages || '').trim();
    const stageIds = stagesRaw
      ? stagesRaw.split(',').map(s => s.trim()).filter(Boolean)
      : null;

    const coloredStagesList = stageIds && stageIds.length ? stageIds : STAGE_FILTER;
    const coloredStagesSet = new Set(coloredStagesList);

    // по умолчанию: прочие стадии -> чёрные
    // если allStages=1 — не черним
    const paintOtherStagesBlack = !forceAllStages;

    const key = cacheKeyOf({
      allStages: forceAllStages,
      coloredStages: coloredStagesList
    });
    ...
    // ✅ ВСЕГДА тянем все стадии, чтобы не терять "чёрные"
    const items = await loadAllSmartItems({ allStages: true, stageIds: null });
    ...
    const mapped = mapItemsToEvents(items, {
      paintOtherStagesBlack,
      coloredStages: coloredStagesSet
    });
